<x-layouts.app>
    ...
    {{-- Unified Table Card — tanpa overflow-hidden agar dropdown tidak terpotong --}}
    <div class="bg-white border border-neutral-200/90 rounded-2xl shadow-xs">
        ...
        <div class="p-4 sm:p-5 border-b border-neutral-200/80 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3.5">
            {{-- Search --}}
            <form method="GET" action="{{ route('surat-masuk.index') }}" class="flex-1 flex items-center gap-2">
                ...
                @if(request('q'))
                    <a href="{{ route('surat-masuk.index') }}" class="px-3.5 py-2 bg-neutral-100 hover:bg-neutral-200 text-neutral-600 font-semibold rounded-xl text-sm border border-neutral-200 transition">
                        Reset
                    </a>
                @endif
            </form>
            ...
            <div class="flex items-center gap-2.5 flex-wrap">
                @if(auth()->check() && auth()->user()->hasPermission('surat_masuk', 'export'))
                    {{-- Ekspor dropdown --}}
                    <div class="relative">
                        <button
                            data-dropdown-btn
                        ...
                        <div data-dropdown-menu class="hidden absolute right-0 mt-2 w-64 bg-white border border-neutral-200 rounded-xl shadow-2xl py-1.5 z-50 text-left">
                            ...
                            <a href="{{ route('surat-masuk.export.pdf', request()->query()) }}" class="flex items-center gap-2.5 px-3.5 py-2 text-xs font-semibold text-neutral-700 hover:bg-danger-50 hover:text-danger-700 transition">
                            ...
                            <a href="{{ route('surat-masuk.export.excel', request()->query()) }}" class="flex items-center gap-2.5 px-3.5 py-2 text-xs font-semibold text-neutral-700 hover:bg-success-50 hover:text-success-700 transition">
                            ...
                            <a href="{{ route('surat-masuk.export.csv', request()->query()) }}" class="flex items-center gap-2.5 px-3.5 py-2 text-xs font-semibold text-neutral-700 hover:bg-primary-50 hover:text-primary-700 transition">
                            ...
                            <a href="{{ route('surat-masuk.print', request()->query()) }}" target="_blank" class="flex items-center gap-2.5 px-3.5 py-2 text-xs font-semibold text-neutral-700 hover:bg-neutral-100 transition">
                ...
                @if(auth()->check() && auth()->user()->hasPermission('surat_masuk', 'create'))
                    <a href="{{ route('surat-masuk.create') }}" class="px-4 py-2 bg-success-600 hover:bg-success-700 text-white font-bold rounded-xl text-sm shadow-xs transition flex items-center gap-1.5 flex-shrink-0">
                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {{-- Bulk Action Bar --}}
        @if(auth()->check() && auth()->user()->hasPermission('surat_masuk', 'delete'))
            <div id="bulk-action-container" class="hidden px-5 py-3 bg-danger-50/80 border-b border-danger-200 flex items-center justify-between gap-4">
                ...
                <form method="POST" action="{{ route('surat-masuk.bulk-destroy') }}" id="bulk-delete-form" data-confirm-delete data-confirm-message="Hapus semua surat masuk yang dipilih?">
                    @csrf
                    @method('DELETE')
        ...
        {{-- ============ TABLE — full width, no clipping ============ --}}
        {{--
            Untuk tabel ≤ 10 baris: otomatis full height tampil semua tanpa scroll.
            Untuk tabel > 10 baris: scroll vertikal diaktifkan (max-h ~600px).
            Scroll horizontal hanya muncul di layar sempit (overflow-x-auto).
        --}}
        <div class="overflow-x-auto {{ $suratMasuk->count() > 10 ? 'max-h-[640px] overflow-y-auto' : '' }}">
            <table class="w-full text-left border-collapse min-w-[760px]">
                <thead class="bg-neutral-50/80 border-b border-neutral-200 text-neutral-500 text-xs font-bold uppercase tracking-wider sticky top-0 z-10">
                    <tr>
                        ...
                        <th class="py-3 px-4 whitespace-nowrap bg-neutral-50/80">Tgl. Terima</th>
                        <th class="py-3 px-4 bg-neutral-50/80">Asal Pengirim</th>
                        ...
                        <tr class="hover:bg-neutral-50/60 transition-colors">
                            @if(auth()->check() && auth()->user()->hasPermission('surat_masuk', 'delete'))
                                <td class="py-3 px-4 text-center">
                                    <input type="checkbox" name="selected_ids[]" value="{{ $sm->id }}" class="row-checkbox w-4 h-4 text-primary-600 rounded border-neutral-300 focus:ring-primary-500 cursor-pointer" />
                                </td>
                            @endif
                            <td class="py-3 px-4 whitespace-nowrap">
                                <span class="px-2.5 py-0.5 bg-primary-50 text-primary-700 font-extrabold text-xs rounded-md border border-primary-200/60">
                                    {{ $sm->no_agenda_formatted }}
                                </span>
                            </td>
                            <td class="py-3 px-4 font-bold text-neutral-900 max-w-[180px]">
                                <span class="block truncate" title="{{ $sm->nomor_surat }}">{{ $sm->nomor_surat }}</span>
                            </td>
                            <td class="py-3 px-4 whitespace-nowrap">
                                <p class="font-semibold text-neutral-800 text-xs">{{ $sm->tanggal_terima?->format('d/m/Y') }}</p>
                                <p class="text-[11px] text-neutral-400">Surat: {{ $sm->tanggal_surat?->format('d/m/Y') }}</p>
                            </td>
                            <td class="py-3 px-4 text-neutral-700 font-medium max-w-[160px]">
                                <span class="block truncate" title="{{ $sm->asal_surat }}">{{ $sm->asal_surat }}</span>
                            </td>
                            <td class="py-3 px-4 text-neutral-600 max-w-[220px]">
                                <p class="line-clamp-2 text-xs leading-relaxed">{{ $sm->perihal }}</p>
                            </td>

                            {{-- Lampiran: 2 link langsung, tanpa dropdown yang bisa tertutup --}}
                            <td class="py-3 px-4 text-center whitespace-nowrap">
                                @if($sm->hasFile())
                                    <div class="flex items-center justify-center gap-1.5">
                                        <a href="{{ route('surat-masuk.download', [$sm, 'type' => 'pdf']) }}"
                                        ...
                                        <a href="{{ route('surat-masuk.download', [$sm, 'type' => 'original']) }}"
                                           title="Unduh file asli (.{{ $sm->original_extension ?: 'file' }})"
                            ...
                            <td class="py-3 px-4 text-center whitespace-nowrap">
                                <div class="flex items-center justify-center gap-1.5">
                                    <a href="{{ route('surat-masuk.show', $sm) }}" class="px-2.5 py-1 bg-neutral-100 hover:bg-primary-50 hover:text-primary-700 text-neutral-700 font-bold rounded-lg text-xs transition">
                                        Lihat
                                    </a>
                                    @if(auth()->check() && auth()->user()->hasPermission('surat_masuk', 'update'))
                                        <a href="{{ route('surat-masuk.edit', $sm) }}" class="px-2.5 py-1 bg-neutral-100 hover:bg-warning-50 hover:text-warning-700 text-neutral-700 font-bold rounded-lg text-xs transition">
                                            Ubah
                                        </a>
                                    @endif
                                    @if(auth()->check() && auth()->user()->hasPermission('surat_masuk', 'delete'))
                                        <form method="POST" action="{{ route('surat-masuk.destroy', $sm) }}" data-confirm-delete data-confirm-message="Hapus Surat Masuk nomor {{ $sm->nomor_surat }}?">
                                            @csrf
                                            @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="p-0">
                                <x-empty-state
                                    title="Data Surat Masuk Tidak Ditemukan"
                                    message="{{ request('q') ? 'Tidak ditemukan data dengan kata kunci tersebut.' : 'Belum ada data surat masuk yang tersimpan.' }}"
                                    action-label="+ Tambah Surat Masuk"
                                    :action-url="auth()->check() && auth()->user()->hasPermission('surat_masuk', 'create') ? route('surat-masuk.create') : ''"
                                />
                            </td>
                        </tr>
                    @endforelse
